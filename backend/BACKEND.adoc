= PyRpiCam OTA Backend Server
:toc: left
:toclevels: 3
:sectlinks:
:sectanchors:

This backend provides server-side infrastructure for the PyRpiCamController Over-The-Air (OTA) update system. It's designed to run in a subfolder `/public_html/pycamota/` alongside other web applications like WordPress.
...
=== Database Schema
* **ota_database_schema.sql**: Complete MySQL database schema with tables for devices, versions, logs, and configuration
* Includes stored procedures for secure operations and sample data

image::../_doc/db_schema.png[]

=== Core PHP Files
* **config.php**: Central configuration with database connection, security settings, and helper functions
* **ota_check.php**: Device update check endpoint - handles version checking and file downloads
* **ota_report.php**: Status reporting endpoint - receives device status updates and handles notifications
